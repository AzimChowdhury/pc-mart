...
import { PowerSupplyContext } from '@/pages/_app';
import { useRouter } from 'next/router';
import Image from 'next/image';
import { useContext } from 'react'

function ChoosePowerSupply({ data }) {
    const router = useRouter()
    const { setPowerSupply } = useContext(PowerSupplyContext)
    return (
            ...
            {data?.map(d => <>
                <div className='w-full flex border-2 border-gray-400 rounded-xl '>
                    ...
                    <div className='ml-3'>
                        ...
                        <button onClick={() => { setPowerSupply(d); router.push('/build') }} className='btn bg-blue-700 hover:bg-blue-800 text-white m-3'>Select</button>
                    </div>
                </div>
            </>)}
        </div>
...
export async function getServerSideProps() {
    const response = await fetch('https://pc-mart-server.vercel.app/powersuppliers');
...
ChoosePowerSupply.getLayout = function getLayout(page) {
    return (
        <RootLayout> {page}
        </RootLayout>
    )
