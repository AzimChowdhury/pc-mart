...
import { RamContext } from '@/pages/_app';
import { useRouter } from 'next/router';
import { useContext } from 'react'

function ChooseRam({ data }) {
    const router = useRouter()
    const { setRam } = useContext(RamContext)
    return (
            ...
            {data.map(d => <>
                <div className='w-full flex border-2 border-gray-400 rounded-xl '>
                    ...
                    <div className='ml-3'>
                        ...
                        <button onClick={() => { setRam(d); router.push('/build') }} className='btn bg-blue-700 hover:bg-blue-800 text-white m-3'>Select</button>
                    </div>
                </div>
            </>)}
        </div>
...
export async function getServerSideProps() {
    const response = await fetch('http://localhost:5000/rams');
...
ChooseRam.getLayout = function getLayout(page) {
    return (
        <RootLayout> {page}
        </RootLayout>
    )
