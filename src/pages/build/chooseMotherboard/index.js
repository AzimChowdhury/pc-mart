...
import { MotherboardContext } from '@/pages/_app';
import { useRouter } from 'next/router';
import { useContext } from 'react'

function ChooseMotherboard({ data }) {
    const router = useRouter()
    const { setMotherboard } = useContext(MotherboardContext)
    return (
            ...
            {data.map(d => <>
                <div className='w-full flex border-2 border-gray-400 rounded-xl '>
                    ...
                    <div className='ml-3'>
                        ...
                        <button onClick={() => { setMotherboard(d); router.push('/build') }} className='btn bg-blue-700 hover:bg-blue-800 text-white m-3'>Select</button>
                    </div>
                </div>
            </>)}
        </div>
...
export async function getServerSideProps() {
    const response = await fetch('http://localhost:5000/motherboards');
...
ChooseMotherboard.getLayout = function getLayout(page) {
    return (
        <RootLayout> {page}
        </RootLayout>
    )
