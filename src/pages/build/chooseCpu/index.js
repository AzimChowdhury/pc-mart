"use  client"
import RootLayout from '@/components/Layout/RootLayout';
import { ProcessorContext } from '@/pages/_app';
import { useRouter } from 'next/router';
import { useContext } from 'react'

function ChooseCpu({ data }) {
    const router = useRouter()
    const { setProcessor } = useContext(ProcessorContext)
    return (
        <div className=' grid grid-cols-2  gap-10 mx-48 my-10'>
            {data.map(d => <>
                <div className='w-full flex border-2 border-gray-400 rounded-xl '>
                    <img src={d.img} alt='' />
                    <div className='ml-3'>
                        <p className='text-2xl font-semibold pr-4 pt-4' >{d.name}</p>
                        <p className='text-xl font-semibold pr-4 pt-2'>price : ${d.price}</p>
                        <button onClick={() => { setProcessor({ name: d.name, price: d.price, id: d._id, img: d.img }); router.push('/build') }} className='btn bg-blue-700 hover:bg-blue-800 text-white mt-3'>Select</button>
                    </div>
                </div>
            </>)}
        </div>
    )
}

export default ChooseCpu



export async function getServerSideProps() {
    const response = await fetch('http://localhost:5000/processors');
    const data = await response.json();
    return {
        props: {
            data
        }
    };
}



ChooseCpu.getLayout = function getLayout(page) {
    return (
        <RootLayout> {page}
        </RootLayout>
    )
}